...
<head>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $requiredFields = [
            ...
            'bumil', 'ibumenyusui', 'pasangansubur', 'wanitausiasubur', 
            'buta', 'makananPokok', 'jaminanKeluarga', 'jumlahJaminan', 
            'sumberAir', 'pembuanganSampah', 'pembuanganAirLimbah', 
            'stiker', 'kriteriaRumah', 'up2k', 'usahaKesling'
        ];
        
        $allFilled = true;
        foreach ($requiredFields as $field) {
            if (empty($_POST[$field]) && !isset($_POST[$field])) { // Check for empty and not set
                $allFilled = false;
                break;
            }
        }
